package com.shangame.fiction.widget.recycler.coverflow;
...
public class CoverFlowLayoutManger extends RecyclerView.LayoutManager {
    ...
    /**Item间隔与item宽的比例*/
    private float mIntervalRatio = 0.75f;
...
//            cm.setSaturation(0.9f);
    ...
    private float computeScale(int x) {
        float scale = 1 - Math.abs(x - mStartX) * 1.0f / Math.abs(mStartX + mDecoratedChildWidth / mIntervalRatio) + 0.18f;
        if (scale < 0) scale = 0;
        if (scale > 1) scale = 1;
        return scale;
    }
