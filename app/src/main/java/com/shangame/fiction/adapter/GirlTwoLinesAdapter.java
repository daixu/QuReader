 ...
 */
public class GirlTwoLinesAdapter extends WrapRecyclerViewAdapter<BookInfoEntity, BookWithContentViewHolder> {

    private int clickType;
    private Activity mActivity;

    public GirlTwoLinesAdapter(Activity activity) {
        mActivity = activity;
        clickType = ApiConstant.ClickType.FROM_CLICK;//默认点击
    }

    public GirlTwoLinesAdapter(Activity activity, int clickType) {
    ...
    public BookWithContentViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view;
        if (viewType == 0) {
            view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_girl_linear_content, parent, false);
        } else {
